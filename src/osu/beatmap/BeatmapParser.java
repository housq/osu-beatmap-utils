...
import osu.io.MD5InputStreamWrapper;
		...
		for (File dir : dirs) {
			...
			for (File file : files) {
				...
				try {
					...
					System.err.println(String.format("Failed to parse beatmap file '%s'.",
						...
						try {
							...
							case "AudioFilename":
								...
								if (!audioFileName.isFile()) {
									...
									if (!match) {
										System.err.println(String.format("Audio file '%s' not found in directory '%s'.", tokens[1], dir.getName()));
										return null;
									}
								}
...
//								break;
							...
							case "LetterboxInBreaks":
								beatmap.letterboxInBreaks = parseBoolean(tokens[1]);
								break;
							case "WidescreenStoryboard":
								beatmap.widescreenStoryboard = parseBoolean(tokens[1]);
								break;
							case "EpilepsyWarning":
								beatmap.epilepsyWarning = parseBoolean(tokens[1]);
								break;
//							case "SpecialStyle":  // mania only
//								beatmap.specialStyle = parseBoolean(tokens[1]);
//								break;
							default:
								break;
							}
						} catch (Exception e) {
							System.err.println(String.format("Failed to read line '%s' for file '%s'.",
...
//							}
//						} catch (Exception e) {
//							System.err.println(String.format("Failed to read editor line '%s' for file '%s'.",
//									line, file.getAbsolutePath()));
//						}
					}
					break;
				case "[Metadata]":
					while ((line = in.readLine()) != null) {
						...
						try {
							...
							System.err.println(String.format("Failed to read metadata '%s' for file '%s'.",
									line, file.getAbsolutePath()));
						}
				...
				case "[Difficulty]":
					while ((line = in.readLine()) != null) {
						...
						try {
							...
							System.err.println(String.format("Failed to read difficulty '%s' for file '%s'.",
									line, file.getAbsolutePath()));
						}
					}
					...
					while ((line = in.readLine()) != null) {
						...
						case "2":  // break periods
							try {
								...
								System.err.println(String.format("Failed to read break period '%s' for file '%s'.",
										line, file.getAbsolutePath()));
							}
							break;
						default:
				...
				case "[TimingPoints]":
					while ((line = in.readLine()) != null) {
						...
						try {
							...
							System.err.println(String.format("Failed to read timing point '%s' for file '%s'.",
									line, file.getAbsolutePath()));
						}
					}
				...
				case "[Colours]":
					...
					while ((line = in.readLine()) != null) {
						...
						try {
							...
							System.err.println(String.format("Failed to read color '%s' for file '%s'.",
									line, file.getAbsolutePath()));
						}
					}
					if (!colors.isEmpty())
				...
				case "[HitObjects]":
					...
					while ((line = in.readLine()) != null) {
						...
						try {
							...
							System.err.println(String.format("Failed to read hit object '%s' for file '%s'.",
									line, file.getAbsolutePath()));
						}
					}
					...
					try {
						...
						System.err.println(String.format("Failed to read hit object end time '%s' for file '%s'.",
								line, file.getAbsolutePath()));
					}
					break;
				default:
			...
			System.err.println(String.format("Failed to read file '%s'.", file.getAbsolutePath()));
		} catch (NoSuchAlgorithmException e) {
			System.err.println("Failed to get MD5 hash stream.");
	...
	public static void parseHitObjects(Beatmap beatmap) {
		...
		try (BufferedReader in = new BufferedReader(new FileReader(beatmap.getFile()))) {
			...
			if (line == null) {
				System.err.println(String.format("No hit objects found in Beatmap '%s'.", beatmap.toString()));
				return;
			}
			...
			while ((line = in.readLine()) != null && objectIndex < beatmap.objects.length) {
				...
				try {
					...
					System.err.println(String.format("Failed to read hit object '%s' for beatmap '%s'.",
							line, beatmap.toString()));
				}
			}

			// check that all objects were parsed
			if (objectIndex != beatmap.objects.length)
				System.err.println(String.format("Parsed %d objects for beatmap '%s', %d objects expected.",
						objectIndex, beatmap.toString(), beatmap.objects.length));
		} catch (IOException e) {
			System.err.println(String.format("Failed to read file '%s'.", beatmap.getFile().getAbsolutePath()));
		}
	}
	...
	private static String[] tokenize(String line) {
		...
		if (index == -1) {
			System.err.println(String.format("Failed to tokenize line: '%s'.", line));
			return null;
		}
